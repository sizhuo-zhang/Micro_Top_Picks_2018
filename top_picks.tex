...
We first study an example race case which involves the instruction issue queue (IQ) in the OOO processor, and then show how CMD extends module interfaces with concurrency properties to guarantee atomicity and solve the problem.

\subsection{Race between Microarchitectural Events}\label{sec:cmd:iq}
Figure shows the modules (blocks in the figure) and microarchitectural events (blobs in the figure) that participate in a race in the OOO processor.
...
Both events are accessing the states in IQ and RDYB, forming a race.


\subsection{Conflict Matrix: Extending Module Interface with Concurrency Properties}
