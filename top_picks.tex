...
Figure~\ref{fig:race:modules} shows the modules (blocks in the figure) and microarchitectural events (blobs in the figure) that participate in a race in the OOO processor.
...
Consider the case that event Rename is checking the source register $R$ of instruction $I$, while event RegWrite is writing register $R$.
It is possible that event Rename first checks RDYB and finds $R$ not ready, then event RegWrite happens and cannot wake up instruction $I$ which is not yet in IQ, and finally event Rename enters $I$ into IQ.
In this case, $I$ will stuck in IQ forever, i.e., the processor deadlocks.
Figure~\ref{fig:race:deadlock} summarizes the deadlock scenario.
...
\begin{figure}[!htb]
    \subfloat[Modules involved in the race\label{fig:race:modules}]{\missingfigure{text}}\\
    \subfloat[Operation sequence that leads to deadlock\label{fig:race:deadlock}]{\missingfigure{text}}
...
That is, event Rename will be expressed as rule Rename which calls method \emph{check} of RDYB and method \emph{enter} of IQ, and event RegWrite will be expressed as rule RegWrite which calls method \emph{set} of RDYB and method \emph{wakeup} of IQ.
...
Back to the race problem in Figure~\ref{fig:race}, if there is no bypass wire in the implementation of module IQ or module RDYB, then the CM of IQ will have method \emph{wakeup} happen before \emph{enter}, and the CM of RDYB will have metho \emph{check} happen before \emph{set}.
...
\section{RiscyOO: Application of CMD to Designing a RISC-V Out-of-Order Processor}
